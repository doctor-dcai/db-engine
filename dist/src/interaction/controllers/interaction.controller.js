...
class InteractionController {
    async createInteraction(req, res) {
        try {
            ...
            const interaction = await interaction_service_1.interactionService.createInteraction(walletAddress, patientProfile, activityLog, summary);
    ...
    async addActivity(req, res) {
        try {
            ...
            const updated = await interaction_service_1.interactionService.addActivityLog(req.params.walletAddress, logEntry);
            if (!updated) {
    ...
    async updateSummary(req, res) {
        try {
            ...
            const updated = await interaction_service_1.interactionService.updateSummary(req.params.walletAddress, summary);
            if (!updated) {
                return res.status(404).json({
                    error: 'Interaction not found'
                });
